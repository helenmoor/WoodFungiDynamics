## 12_FitModels_CEM_mature_forProjs.R
# HM 200610, helenmoor@gmx.ch

# Fits the colonization-extinction models (CEMs) used in projections for all species (see SI, table S6);
# Differences to main models: 
# - Local connectivtiy is not included as a covariate
# - Mean decay stage is not included as covariate for tricabie
...
OUTDIR <- "./MODELS/ForProjections/CEM_mature/"

dir.create("MODELS/ForProjections/")
dir.create("MODELS/ForProjections/CEM_mature/")
...
colparmslist <- list( c("DWVol_m3perHa_T2_SpruceConif", "AgeT2"), # amylapp, stand age instead of local connectivity
                   ...
                   c("DWVol_m3perHa_T2_SpruceConif"), # phlecent, covariates remain the same without local connectivity
...
for(myloop in 1:length(modelspecies)){
  ...
  ## Indata to model:-----------------------------------
    ...
    } # finish model fitting with Pext ~ 1
